...
export function billingBody(settings: PlansConfig, reconciliation?: Reconciliation): string {
  ...
  <p>Amounts are USD. Enabled plans must match their Stripe Price exactly. Price IDs are verified on save and checkout. Create a new Price in Stripe to change what new customers pay.</p>
  <form method="post" action="/billing/config" class="panel" style="display:grid;gap:1rem;max-width:42rem">
    ${(['monthly', 'annual'] as const).map((plan) => `<fieldset>
      <legend>Premium ${plan} — ${settings[`${plan}Enabled`] ? 'Enabled' : 'Disabled'}</legend>
  ...
  <p>One-time purchases are reserved for a future release and cannot be enabled yet.</p>
